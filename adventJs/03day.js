...
function distributeGifts(packOfGifts, reindeers) {
    let weigthOfPack = packOfGifts.reduce(getSum, 0);
    let capacityOfReindeers = reindeers.reduce(getSum, 0);
    function getSum(total, num) {
        return total + num.length;
    }
    capacityOfReindeers *= 2;
    return Math.floor(capacityOfReindeers / weigthOfPack);
    }
